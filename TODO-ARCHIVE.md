...
> Closed and retired records, moved verbatim out of `TODO.md`. See
> [`TODO.md`](TODO.md) for open, actionable work.


> **CLOSED 2026-09-06, same day it was filed.** The working-tree `cp`
> re-vendor (`//go:generate cp ../../../issuer/docs/swagger.yaml openapi.yaml`)
> is gone: `scripts/revendor-spec.sh` vendors from a release tag only,
> `cmd/realm-id/ISSUER_CONTRACT` records which one, and
> `cmd/realm-id/spec_contract_test.go` proves the embedded bytes are that one
> and still yield a real command tree. Cross-repo alignment with the BFF is
> enforced by the umbrella's `scripts/issuer-pin-parity.py`. See
